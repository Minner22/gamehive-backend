...
- V8 join tables: `ON DELETE CASCADE` on the `game_id` side only; the dictionary side is RESTRICT — so once real games exist, #115's taxonomy DELETE endpoints hit a constraint violation for in-use entries (handle via the pre-existing `*_IN_USE` codes, planned for #117).
...
- `GameMapper` is the first mapper with `uses = {...}` (delegates collection elements to the #114 mappers; MapStruct converts `Set<Entity> → List<Dto>` automatically). `GameDto` exposes `moderationStatus` + `rejectionReason` but **not** `submittedBy`/`reviewedBy`/`reviewedAt`/`resubmissionCount` (moderation-internal; #118 gets its own view). Mapping touches all four LAZY collections — #117 must add an `@EntityGraph` finder before mapping outside a transaction. List order is nondeterministic (`HashSet` source).
