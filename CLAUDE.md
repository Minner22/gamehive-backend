...
- `game/` — "Gry" phase (roadmap `docs/roadmap-games.md`): taxonomy dictionaries `Publisher`/`Category`/`Mechanic`/`Author` (#114) + `TaxonomyAdminController` (#115) + the `Game` entity with moderation workflow (#116, data layer only — endpoints come in #117/#118). Same `model/`/`repository/`/`service/`/`mapper/`/`dto/`/`controller/` split as `user/`.
...
### Games & moderation workflow (data layer, GH-116)
...
- `Game` (`game/model`, table `games`, migration `V8`) has four `@ManyToMany` sets (LAZY, **no cascade** — dictionaries have independent lifecycles) through singular-named join tables `game_publisher`/`game_category`/`game_mechanic`/`game_author` with composite PKs. Construction via **Lombok `@Builder` on the private constructor** (deliberate new pattern: 9 positional params incl. 5 consecutive ints; a *class-level* `@Builder` would not see the inherited `submittedBy`); collections are populated via `addPublisher/addCategory/...`. "≥1 publisher/category" and `min <= max` are **deliberately not enforced** in entity or DB — service validation in #117.
- V8 join tables: `ON DELETE CASCADE` on the `game_id` side only; the dictionary side is RESTRICT (plus dictionary-side indexes `idx_game_*_*_id` — Postgres doesn't auto-index FK columns) — so once real games exist, taxonomy DELETE of an in-use entry hits a constraint violation (handle via the pre-existing `*_IN_USE` codes, planned for #117; the `TODO(#117)` markers sit in `TaxonomyServiceImpl`). Fixture consequence: **all three seeded publishers are game-referenced**, so taxonomy delete-tests must target unreferenced entries (category 3) or freshly created ones.
...
- `GameMapper` follows `UserMapper`'s `uses = {...}` pattern (delegates collection elements to the #114 mappers; MapStruct converts `Set<Entity> → List<Dto>` automatically). `GameDto` exposes `moderationStatus` + `rejectionReason` but **not** `submittedBy`/`reviewedBy`/`reviewedAt`/`resubmissionCount` (moderation-internal; #118 gets its own view). Mapping touches all four LAZY collections — for #117 prefer `@BatchSize`/`default_batch_fetch_size` over a four-collection `@EntityGraph` (fetching 4 sets in one query is a cartesian product: rows = publishers × categories × mechanics × authors per game; an `@EntityGraph` is fine for at most one collection). List order is nondeterministic (`HashSet` source).
- `GameRepository` shapes anticipate the known query patterns: `findByTitle` returns `List` (title is deliberately **not unique** — the moderation model keeps rejected duplicates alongside approved originals, so an `Optional` finder would throw on the first duplicate), `findByModerationStatus` is `Page`-only (for APPROVED it's the entire global library).
