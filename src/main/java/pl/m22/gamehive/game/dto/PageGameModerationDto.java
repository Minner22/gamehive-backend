...
/**
 * Schemat dokumentacyjny stronicowanej kolejki moderacji.
 * Odwzorowuje płaski JSON {@code PageImpl} z wymaganymi polami — nie jest serializowany,
 * służy wyłącznie do wygenerowania kontraktu OpenAPI (zastępuje słaby auto-schemat {@code Page<GameModerationDto>}).
 */
@Schema(description = "Stronicowana lista zgłoszeń w widoku moderacji.")
public record PageGameModerationDto(

        @Schema(description = "Zawartość bieżącej strony.", requiredMode = Schema.RequiredMode.REQUIRED)
        List<GameModerationDto> content,
